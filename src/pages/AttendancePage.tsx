...
const AttendancePage = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((error) =>
        console.error("社員リストの読み込みに失敗しました。", error),
      );
    ...
    axios
      ...
      .catch((error) =>
        console.error("出席データ読み込みに失敗しました。", error),
      );
  }, []);

  const filteredAndSortedEmployees = useMemo(() => {
    return employeeList
      .filter((emp) => selectedDept === "すべて" || emp.dname === selectedDept)
      .sort((a, b) => {
        const deptCompare = a.dname.localeCompare(b.dname, "ja");
  ...
  const toggleAttendance = (
    ...
    axios
      .post("http://localhost:5000/api/attendance/toggle", { empno, deptno })
      .then((res) => {
        // setAttendanceMap(res.data.attendanceMap);

        const { isIn, arrivalTime } = res.data;
        setAttendanceMap((prev) => {
          const updated = {
            ...prev,
            [empno]: {
              isIn: isIn,
              arrivalTime: isIn ? arrivalTime : prev[empno]?.arrivalTime,
            },
          };
          localStorage.setItem("attendance-status", JSON.stringify(updated));
          return updated;
        });
        if (isIn) {
    ...
    <PageLayout title="出席管理">
      ...
      <div className="grid grid-cols-1 overflow-y-auto sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredAndSortedEmployees.map((emp) => {
          const record = attendanceMap[emp.empno] || { isIn: false };
          return (
            <div
              key={emp.empno}
              className={`p-6 m-1 bg-white rounded-2xl border transition-all ${record.isIn ? "border-zinc-800 shadow-md ring-1 ring-zinc-800" : "border-zinc-200"}`}
            >
              <div className="flex justify-between items-start mb-2">
                <div>
                  <h3 className="font-bold text-lg">{emp.ename}</h3>
              ...
              <button
                onClick={() =>
                  toggleAttendance(emp.empno, emp.ename, emp.deptno)
                }
